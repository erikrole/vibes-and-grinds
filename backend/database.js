...
async function initDatabase() {
  ...
  const vestMigrations = [
    { name: 'espn_event_id', ddl: 'ALTER TABLE vest_games ADD COLUMN espn_event_id TEXT' },
  ];

  for (const column of vestMigrations) {
    if (!vestColumnNames.has(column.name)) {
      await db.exec(column.ddl);
    }
  }
